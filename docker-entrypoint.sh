...
echo ENABLE_STATSD_METRICS=$ENABLE_STATSD_METRICS
echo Running : $1
...
if [ "$1" = "--help" -o "$1" = "-h" ]; then
    ...
    if [ "$ENABLE_STATSD_METRICS" = true ]; then
       echo 'Replacing Content'
       sed -i -e "s/#metrics.reporters:/metrics.reporters: statsd/g" $FLINK_HOME/conf/flink-conf.yaml
       sed -i -e "s/#metrics.reporter.stsd.class: org.apache.flink.metrics.statsd.StatsDReporter/metrics.reporter.stsd.class: org.apache.flink.metrics.statsd.StatsDReporter/g" $FLINK_HOME/conf/flink-conf.yaml
       sed -i -e "s/#metrics.reporter.stsd.host: localhost/metrics.reporter.stsd.host: ${STATSD_SERVER}/g" $FLINK_HOME/conf/flink-conf.yaml
       sed -i -e "s/#metrics.reporter.stsd.port: 8125/metrics.reporter.stsd.port: ${STATSD_PORT}/g" $FLINK_HOME/conf/flink-conf.yaml
